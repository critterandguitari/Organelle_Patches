...
mv UpdateOS-3.0/manifest.txt /tmp
find UpdateOS-3.0 -type f ! -name "._*" -print0  | xargs -0 sha1sum > /tmp/sha1sum.txt
sort /tmp/manifest.txt > /tmp/manifest.orig
sort /tmp/sha1sum.txt  > /tmp/manifest.new
diff /tmp/manifest.orig /tmp/manifest.new; ec=$?;

IMAGE_DIR=${PATCH_DIR}/UpdateOS-3.0
cd ${IMAGE_DIR}
